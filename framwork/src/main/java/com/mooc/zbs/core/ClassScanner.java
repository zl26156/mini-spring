...
public class ClassScanner {
    //获取jar包的绝对路径，再根据路径获取jar包下所有类的class对象
    ...
    private static List<Class<?>> getClassesFromJar(String jarFilePath,String path) throws IOException, ClassNotFoundException {
        ...
        Enumeration<JarEntry> jarEntries = jarFile.entries();//获取jia包下entries的集合
        while(jarEntries.hasMoreElements()){
            JarEntry jarEntry = jarEntries.nextElement();
            String entryName = jarEntry.getName();// entryName长这样 com/mooc/zbs/test/test.class
            if(entryName.startsWith(path)&&entryName.endsWith(".class")){
                String classFullName = entryName.replace("/",".").substring(0,entryName.length()-6);
                classes.add(Class.forName(classFullName));//将class加载到jvm里
            }
        }
        return classes;
    }
